...
{
	bool arg_starts_correctly = strncmp(arg, "$", 1) == 0;
	bool envar_no_name =
	        strncmp(arg, "$\0", 2) == 0 || strncmp(arg, "$ ", 2) == 0;
	...
	if (arg_starts_correctly) {
		...
		char *strip_env = arg + 1;
		char *env = getenv(strip_env);

		size_t env_size;
		if (!env || (env_size = strlen(env)) == 0) {
			return NULL;
		}

		if (strlen(arg) < env_size) {
			arg = realloc(arg, 1 + sizeof(char) * env_size);
		}

		return strcpy(arg, env);
	}
	return arg;
}
